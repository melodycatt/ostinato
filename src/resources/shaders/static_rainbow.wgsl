fn hash(p: vec3<f32>) -> f32 {
    let h = dot(p, vec3<f32>(12.9898, 78.233, 34.852));
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let nx = hash(vec3<f32>(floor(in.position.xy / pixel_size) * pixel_size, time.time * speed) * scale);
    let ny = hash(vec3<f32>(floor(in.position.yw / pixel_size) * pixel_size, time.time * speed) * scale);
    let nz = hash(vec3<f32>(floor(in.position.xw / pixel_size) * pixel_size, time.time * speed) * scale);
    let n = sin(in.position.w * 2.0);
    return vec4<f32>(nz, nz, nz, 1.0);
}