...
use crate::{camera::{controller::CameraController, light::LightUniform, Camera, CameraConfig}, input::{keyboard::KeyboardData, mouse::MouseData}, mesh::new_cube, resources::{load_model, load_texture, Resource}, texture::Texture};
...
impl State {
    ...
    async fn init(&mut self) -> anyhow::Result<()> {
        ...
        let march = RaymarchUniform {
            ...
            delta: start.elapsed().as_secs_f32() / 1000.,
    ...
    fn update(&mut self) {
        ...
        // no real reason to have a block anymore, but it could be useful and its harmless
        {
            ...
            march.delta = self.delta().elapsed().as_secs_f32() / 1000.;
